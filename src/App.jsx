  ...
  const S = {
    ...
    try {
      ...
      if (profile) {
        ...
        // Fallback ke USERS demo jika profile belum ada
        const demoUser = USERS.residents.find(r => r.email === loginForm.email)
          || (loginForm.email === USERS.supervisor.email ? USERS.supervisor : null);
        if (demoUser) setCurrentUser({ ...data.user, ...demoUser, role: demoUser.role || "resident" });
        else setCurrentUser({ ...data.user, role: loginForm.role });
      }
    } catch (err) {
      if (err.message.includes("Invalid login")) {
        alert("Email atau password salah. Coba lagi.");
      } else if (err.message.includes("Email not confirmed")) {
        alert("Email belum dikonfirmasi. Cek inbox email Anda untuk link konfirmasi.");
      } else {
        // Fallback ke demo login jika Supabase belum dikonfigurasi
        if(loginForm.role==="supervisor"&&loginForm.email==="supervisor@fkusu.ac.id") {
          ...
          if(r) setCurrentUser({...r,role:"resident"});
          else alert("Email tidak ditemukan: " + err.message);
        }
      }
    }
  };
